...
repo_path = r'lib\data\repositories\categories_repository.dart'

repo_old = "  Stream<List<CategoryModel>> watchActiveCategories() {"
...
if repo_old not in content:
    print(f'[ERRO] Trecho não encontrado em {repo_path}')
else:
    content = content.replace(repo_old, repo_new, 1)
    with open(repo_path, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f'[OK] {repo_path}')

# ── 2. categories_page.dart ────────────────────────────────────────────────────

page_path = r'lib\features\categories\categories_page.dart'
